param(
    # [string]$Root = "\\SynologyDS920\home\Photos",
    [string]$Root = "\\SynologyDS920\home\Photos",
    [string]$Progress = "",
...
function Get-PythonLauncher {
    ...
    foreach ($candidate in $candidates) {
        $resolved = Find-CommandPath $candidate
        if ($resolved) {
            switch ($candidate) {
                "py" {
                    return @("py", "-3.11")
    ...
    throw "Python 3.11+ was not found. Install Python 3.11 and make sure 'python3.11', 'python', or 'py' is available in PATH."
}
...
try {
    ...
    if ($pythonPrefixArgs.Count -gt 0) {
        Write-Step "Python launcher args: $($pythonPrefixArgs -join ' ')"
    }
